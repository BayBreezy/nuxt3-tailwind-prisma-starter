<template>
  <div>
    <HListbox :name="name" v-model="value">
      <div class="relative">
        <HListboxLabel class="label" v-if="label">{{ label }}</HListboxLabel>
        <HListboxButton class="input relative text-left" type="text">
          <span v-if="!value" class="text-sm opacity-60">{{
            placeholder
          }}</span>
          <span v-else class="text-sm">{{ selectedValue }}</span>
          <span
            class="absolute inset-y-0 right-3 flex items-center justify-center opacity-50"
          >
            <Icon size="16" name="ph:caret-down" />
          </span>
        </HListboxButton>
        <TransitionSlide :offset="[0, '-10%']">
          <HListboxOptions
            class="absolute left-0 z-30 mt-1.5 max-h-52 w-full space-y-2 overflow-y-auto rounded-md bg-white p-1 shadow-lg ring-1 ring-black/10 focus:outline-none dark:bg-gray-900"
          >
            <HListboxOption
              v-slot="{ active, selected }"
              as="template"
              :key="item as any"
              :value="isObject ? item[keyVal] : item"
              v-for="item in items"
            >
              <li
                :class="[{ 'bg-primary text-white': active }]"
                class="relative w-full cursor-pointer rounded p-2 transition-colors"
              >
                <span class="text-sm">{{
                  isObject ? item[labelVal] : item
                }}</span>
                <span
                  v-if="selected"
                  class="absolute inset-y-0 right-3 flex items-center justify-center"
                >
                  <Icon
                    :class="[active ? 'text-white' : 'text-primary']"
                    name="ph:check-circle"
                    size="22"
                  />
                </span>
              </li>
            </HListboxOption>
          </HListboxOptions>
        </TransitionSlide>
      </div>
    </HListbox>
    <p class="mt-1 text-xs">
      &nbsp;
      <TransitionSlide group mode="out-in" :offset="[0, 20]">
        <span class="text-gray-500" v-if="hint && !errorMessage">{{
          hint
        }}</span>
        <span class="text-red-500 dark:text-red-400" v-if="errorMessage">{{
          errorMessage
        }}</span>
      </TransitionSlide>
    </p>
  </div>
</template>

<script lang="ts">
export default { inheritAttrs: false };
</script>

<script setup lang="ts">
import { useField } from "vee-validate";
const props = defineProps({
  label: String,
  items: {
    type: Array,
    default: () => [],
  },
  hint: String,
  placeholder: {
    type: String,
    default: "Select an option",
  },
  modelValue: null,
  name: {
    type: String,
    required: true,
  },
  rules: null,
  keyVal: {
    type: String,
    default: "id",
  },
  labelVal: {
    type: String,
    default: "text",
  },
});

const { errorMessage, value } = useField(ref(props.name), props.rules, {
  validateOnValueUpdate: true,
  label: props.label,
  initialValue: props.modelValue,
});

//check if items is an array of objects
const isObject = props.items.length > 0 && typeof props.items[0] === "object";

// get selected value
const selectedValue = computed(() => {
  if (isObject) {
    const selected = props.items.find(
      (item: any) => item[props.keyVal] === value.value
    );
    return selected ? selected[props.labelVal] : "";
  }
  return value.value;
});
</script>
